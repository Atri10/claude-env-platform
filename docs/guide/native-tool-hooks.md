...
|---|---|---|---|
| `CLAUDE_ENV_HOME` | path | `~/.claude-env` | Where hooks add both themselves and the repo checkout to `sys.path`, and where `INCIDENT_MARKER` (`state/INCIDENT`) is looked up (`hooks/policy_hook.py:53,61`; `hooks/audit_hook.py:22`). |
| `CLAUDE_ENV_HOOK_FAIL_CLOSED` | bool-ish string | unset (`false`) | If `"true"` (case-insensitive), an internal error in `policy_hook.py`'s `main()` denies the call instead of allowing it (`hooks/policy_hook.py:62`, `546-550`). Recommended for tier-2+ machines per the module docstring. |
| `CLAUDE_ENV_MCP_FIRST` | bool-ish string | unset (`true`) | If `"false"`, disables the MCP-first redirect entirely — `_mcp_first_hint()` returns `None` unconditionally (`hooks/policy_hook.py:115-116`). |
| `CLAUDE_ENV_HOOK_AUDIT_ALL` | bool-ish string | unset (`false`) | If `"true"`, `audit_hook.py` writes a row for **every** intercepted tool, not just the mutating set (`hooks/audit_hook.py:28,36`). |

| Installer constant | Value | Effect |
|---|---|---|
| `PRE_MATCHER` (`hooks/install_hooks.py:26`) | `"Read\|Write\|Edit\|NotebookEdit\|Glob\|Grep\|Bash"` | Which tools trigger `policy_hook.py` on `PreToolUse`. |
| `POST_MATCHER` (`hooks/install_hooks.py:27`) | `"Write\|Edit\|NotebookEdit\|Bash"` | Which tools trigger `audit_hook.py` on `PostToolUse`. Note `Read`/`Glob`/`Grep` are **not** in this matcher at all — `audit_hook.py`'s own `_MUTATING` filter is a second, redundant layer of the same restriction. |
...
`_PATH_KEYS = ("file_path", "path", "notebook_path")` (`hooks/policy_hook.py:65`) covers
...
(`hooks/policy_hook.py:31-38`). The first subtlety is that plain `shlex.split()` glues
operators onto adjacent words:

```python
# hooks/policy_hook.py:163-178
...
This was a real, fixed bypass: `tests/test_policy_hook_bash.py:200-213`
...
"&&", "||", "|&", "\n", "(", ")", "{", "}"}` (`hooks/policy_hook.py:153`) so each
...
(`test_candidates_ignore_flags_and_bare_nonexistent`, `tests/test_policy_hook_bash.py:70-73`):
...
command (`test_candidates_urls_are_not_paths`, `tests/test_policy_hook_bash.py:64-67`).
...
"install", "ln", "rsync"}` (`hooks/policy_hook.py:236`). For `cp`/`mv`/`install`/`ln`/
...
`key=value` argument style (`hooks/policy_hook.py:270-271`). This is why `cat
...
(`tests/test_policy_hook_bash.py:188-192`).

### `_mutating_reason()` — the hard-deny list, independent of path

```python
# hooks/policy_hook.py:282-304
...
`_MUTATING_CMDS` (`hooks/policy_hook.py:142-146`) is `rm rmdir unlink shred dd mkfs
...
(`test_allow_readonly_and_additive_commands`, `tests/test_policy_hook_bash.py:105-109`).
Additive commands (`mkdir`, `touch`, `cp`, `ln`) are intentionally excluded from
`_MUTATING_CMDS` per the inline comment at `hooks/policy_hook.py:140-141` — "to avoid
over-blocking."

### `_inspect_bash()` — the six-step precedence chain

```python
# hooks/policy_hook.py:307-313
...
(`hooks/policy_hook.py:325-342`), even though the docstring's one-line summary omits
it. In full:
...
(`hooks/policy_hook.py:331-342`) and the native `Write`/`Edit`/`NotebookEdit` path in
`main()` (`hooks/policy_hook.py:480-498`). Both call sites pass **both** the
...
`hooks/policy_hook.py:519-520`) and can be switched off wholesale with
`CLAUDE_ENV_MCP_FIRST=false`. `tests/test_policy_hook_bash.py:122-129`
...
Putting it together, in the order the real code checks them (`hooks/policy_hook.py:421-552`):
...
4. `PolicyEngine.load(root)` where `root` is found by walking up from `cwd` looking
   for `.claude/repo-policy.yaml` or `.git` (`_repo_root`, `hooks/policy_hook.py:399-405`).
...
`audit/audit_logger.py:124-129`) per call, with `tool` set to `f"native.{tool}"` (e.g.
...
(`hooks/audit_hook.py:27`) is the default filter; every arg value is truncated to
...
- **The docstring's stated precedence is incomplete.** `_inspect_bash()`'s own
  ...
  egress > secret in the command string" (`hooks/policy_hook.py:311-312`), but the real
  code has a control-plane write check between "destructive" and "denied path" that
  the one-line summary never mentions (`hooks/policy_hook.py:325-342`). Read the code,
...
- **Reading the control plane is always fine; only writes are guarded.** `cat
  ...
  (`test_control_plane_read_is_allowed`, `tests/test_policy_hook_bash.py:188-192`) —
...
- **The hard-deny list intentionally excludes additive commands.** `mkdir`, `touch`,
  ...
  (`hooks/policy_hook.py:140-141` comment) — `cp src/app.py src/copy.py` is allowed
  outright by `test_allow_readonly_and_additive_commands`
  (`tests/test_policy_hook_bash.py:105-109`), even though `cp` can overwrite an
...
- **The audit row for a Bash denial happens *before* the deny is printed, and failure
  to write it is swallowed.** `hooks/policy_hook.py:461-469` wraps the
  ...
  (`hooks/policy_hook.py:485-492`, `504-511`, `534-542`); the inline comment at
  `hooks/policy_hook.py:511` states the intent directly: "auditing must never break the
  decision itself."
- **Fail-open is the default for the *entire* `main()` body, not just the engine
  load.** The `try` at `hooks/policy_hook.py:451` wraps everything from
  ...
  falls through to `except Exception as exc:` at line 546, which allows silently unless
...
- **The `PostToolUse` matcher and the hook's own `_MUTATING` set are redundant by
  ...
  (`hooks/install_hooks.py:27`), so `audit_hook.py`'s internal `_MUTATING` check
  (`hooks/audit_hook.py:27,36`) only actually does work when `CLAUDE_ENV_HOOK_AUDIT_ALL`
...
- **`Write` content is never stored in the audit ledger, even truncated.**
  ...
  (`hooks/audit_hook.py:44-46`). The ledger records *that* a write happened and to
...
- **`_repo_root()` prefers an existing repo policy file over a bare `.git`.** Because
  ...
  ".git").exists()` at each directory (`hooks/policy_hook.py:402-404`) walking
