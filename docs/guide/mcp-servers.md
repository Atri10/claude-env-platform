...
Each entry under `mcpServers` sets a numeric `startup_order` (`config/mcp-servers.json:16,
...
`jetbrains` (order 7, `config/mcp-servers.json:167-183`), is IDE-provided rather than a
script under `mcp-servers/` and isn't covered further here. The `scopes` array per
server (e.g. `config/mcp-servers.json:22-26` for filesystem-policy) is documentation of
...
```python
# mcp-servers/filesystem-policy/server.py:31-34 (identical pattern in all six)
...
```python
# mcp-servers/lancedb-rag/server.py:102-109 (same shape in all six)
async def _run() -> None:
...
| | `filesystem.write` | `path`, `content` (both required) | Re-evaluates the *destination* path AND scans the outgoing payload for secrets before writing — a write to an allowed path with secret content is still refused (`mcp-servers/filesystem-policy/server.py:155-160`). |
| | `filesystem.list` | `path` (required) | Silently omits any child path whose `evaluate_path` is `block` — blocked entries don't even appear in the listing (`server.py:178-181`). |
...
| **lancedb-rag** | `lancedb.search` | `query` (required), `repo`, `branch`, `top_n` (1-20, optional) | Every hit is screened by `RagPoisonDetector.scan_chunk` before being returned; blocked hits are dropped, not surfaced (`mcp-servers/lancedb-rag/server.py:88-96`). |
...
| | `memory.write` *(conditional)* | `memory_type`, `node_kind`, `name`, `body` (all required), `repo`, `confidence` | Only registered/listed at all when `CLAUDE_ENV_MEMORY_WRITE=true` (`server.py:94-124`); `node_kind` must be one of `HALF_LIFE`'s keys and is cross-checked against `memory_type`'s taxonomy — see [`memory-graph.md`](memory-graph.md). |
| | `memory.link` *(conditional)* | `src`, `dst`, `rel` (required), `weight` | Same `WRITE_ENABLED` gate as `memory.write`. No `memory.delete`/`memory.prune` tool exists in this server at all — destructive ops live only in the `memory_pruner` CLI behind the approval gate (`server.py:11-12`). |
| **terminal** | `terminal.run_tests` | *(none)* | Runs only if `.claude/commands.json` explicitly sets `run_tests`; otherwise returns a "NOT CONFIGURED" directive and runs nothing (`mcp-servers/terminal/server.py:214-223`). |
...
| | `documentation.fetch` *(conditional)* | `url` (required) | Only appended to `list_tools()`'s return value when `FETCH_ALLOWED` (`TIER in (0, 1)`, `mcp-servers/documentation/server.py:42,81-89`) — in tier 2/3 repos the tool doesn't exist in the schema at all, not merely "denied at call time." |
...
- **filesystem-policy** — it is *the* sanctioned path to disk (see
  ...
  (`mcp-servers/filesystem-policy/server.py:66-73`).
- **git** — defense in depth via a plain substring denylist checked *before* any
  ...
  "filter-branch", "remote add", "config")` (`mcp-servers/git/server.py:41-42`), matched
...
- **lancedb-rag** — read-only by construction: the module docstring states indexing is
  ...
  (`mcp-servers/lancedb-rag/server.py:10-11`); the only tool is `lancedb.search`, and
...
- **documentation** — the only server with any outbound network capability, and it's
  ...
  `list_tools()` (`mcp-servers/documentation/server.py:42,81-89`). A tier-2/3 agent
...
- **`call_tool()` never leaks a raw exception to the agent.** filesystem-policy wraps
  ...
  (`mcp-servers/filesystem-policy/server.py:126-129`); memory-graph does the same with a
...
- **`terminal`'s environment is scrubbed for every execution path**, both the
  ...
  "LC_ALL", "TMPDIR", "VIRTUAL_ENV", "PWD"}` (`mcp-servers/terminal/server.py:73`) — no
...
- **`documentation.fetch`'s tier comes from an env var the server trusts as-is.**
  `TIER = int(os.environ.get("CLAUDE_ENV_TIER", "1"))` (`server.py:39`) — this server
  ...
  (`mcp-servers/terminal/server.py:103-114`), which reads the tier directly out of
...
- **`memory-graph` degrades gracefully without an embedder.** If `rag.config.get_embedder()`
  ...
  (`mcp-servers/memory-graph/server.py:41-46`).
- **`git.commit` does not exist as a tool in this server**, despite
  `config/mcp-servers.json:63-64` listing `git.commit` under `requires_approval`. The
  `scopes` array (`config/mcp-servers.json:48-53`) only lists `log`, `diff`, `blame`,
...
- **`lancedb-rag` and `memory-graph` both cache expensive objects at module scope.**
  ...
  loads once per pair (`mcp-servers/lancedb-rag/server.py:45-54`); `memory-graph`
