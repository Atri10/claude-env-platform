...
def _is_within(path: Path, root: Path) -> bool:
    """True if resolved `path` is `root` itself or nested under it."""
    return path == root or root in path.parents


def confine_to_roots(command: str, cwd: str, roots: list[Path]) -> CommandVerdict:
    """Structural, deny-by-default path confinement -- independent of, and in
    ADDITION to, inspect_command()'s named-pattern deny check above.

    inspect_command()'s step 1 only blocks a file argument that matches a
    CONFIGURED deny pattern (.env, secrets/**, .ssh/**, ...). That blocklist
    was designed for filesystem.read/write, which can only ever touch paths
    already inside the repo. terminal.run_scratch instead spawns a real OS
    process whose arguments can name ANY path the machine's user can read --
    e.g. `cat ~/.aws/credentials` matches no configured deny pattern at all,
    so inspect_command() alone would allow it straight through.

    confine_to_roots() flips the model for that gap: every file-looking
    candidate path in `command` must resolve INSIDE at least one of `roots`
    (e.g. REPO_ROOT, SCRATCH_ROOT) or the whole command is denied, regardless
    of what the path is named. This does not replace the named-pattern check
    -- callers should run both (see mcp-servers/terminal/server.py's
    _run_scratch); whichever denies first wins.
    """
    paths, _nets = bash_candidates(command, cwd)
    resolved_roots = [_resolve(Path(r)) for r in roots]

    for tok in paths:
        expanded = os.path.expanduser(tok)
        abs_tok = expanded if os.path.isabs(expanded) else str(Path(cwd or ".") / expanded)
        resolved = _resolve(Path(abs_tok))
        if any(_is_within(resolved, root) for root in resolved_roots):
            continue
        return CommandVerdict(
            "deny",
            f"path '{tok}' resolves outside the allowed directories for this "
            f"command ({', '.join(str(r) for r in resolved_roots)}) — commands "
            f"may only touch files inside the repo or its scratch directory, "
            f"regardless of the path's name",
            tok)
