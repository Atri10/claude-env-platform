...
**Source:** `claudenv/adapters/mcp/{filesystem,git,lancedb_rag,memory_graph,terminal,documentation}/server.py`
(each ~90-270 lines). **Config:** [`claudenv/_data/config/mcp-servers.json`](../../claudenv/_data/config/mcp-servers.json).
...
no HTTP, no port, no socket. Six server scripts live under `claudenv/adapters/mcp/`, registered
in [`claudenv/_data/config/mcp-servers.json`](../../claudenv/_data/config/mcp-servers.json), each launched with the
...
## Startup & registration (`claudenv/_data/config/mcp-servers.json`)
...
`jetbrains` (order 7, `claudenv/_data/config/mcp-servers.json`), is IDE-provided rather than a
script under `claudenv/adapters/mcp/` and isn't covered further here. The `scopes` array per
server (e.g. `claudenv/_data/config/mcp-servers.json` for filesystem-policy) is documentation of
...
```python
# claudenv/adapters/mcp/filesystem-policy/server.py (identical pattern in all six)
...
This is why editing this repo's `claudenv/adapters/mcp/*/server.py` doesn't change live behavior —
the running process imports from `$CLAUDE_ENV_HOME`, a mirror created by
`claude-env init` (see the `claude-env-development` skill / root `CLAUDE.md`).
...
```python
# claudenv/adapters/mcp/lancedb-rag/server.py (same shape in all six)
async def _run() -> None:
...
| | `filesystem.write` | `path`, `content` (both required) | Re-evaluates the *destination* path AND scans the outgoing payload for secrets before writing — a write to an allowed path with secret content is still refused (`claudenv/adapters/mcp/filesystem/server.py`). Same `scratch://` scheme as `filesystem.read`. |
...
| **lancedb-rag** | `lancedb.search` | `query` (required), `repo`, `branch`, `top_n` (1-20, optional) | Every hit is screened by `RagPoisonDetector.scan_chunk` before being returned; blocked hits are dropped, not surfaced (`claudenv/adapters/mcp/lancedb_rag/server.py`). |
...
| **terminal** | `terminal.run_tests` | *(none)* | Runs only if `.claude/commands.json` explicitly sets `run_tests`; otherwise returns a "NOT CONFIGURED" directive and runs nothing (`claudenv/adapters/mcp/terminal/server.py`). |
...
| | `documentation.fetch` *(conditional)* | `url` (required) | Only appended to `list_tools()`'s return value when `FETCH_ALLOWED` (`TIER in (0, 1)`, `claudenv/adapters/mcp/documentation/server.py,81-89`) — in tier 2/3 repos the tool doesn't exist in the schema at all, not merely "denied at call time." |
...
- **filesystem-policy** — it is *the* sanctioned path to disk (see
  ...
  (`claudenv/adapters/mcp/filesystem/server.py`). A `scratch://` path uses the identical
  ...
  atexit/signal handlers plus a `claude-env init` TTL-reaper backstop. Read/write/list
...
- **git** — defense in depth via a plain substring denylist checked *before* any
  ...
  "filter-branch", "remote add", "config")` (`claudenv/adapters/mcp/git/server.py`), matched
...
- **lancedb-rag** — read-only by construction: the module docstring states indexing is
  ...
  (`claudenv/adapters/mcp/lancedb_rag/server.py`); the only tool is `lancedb.search`, and
...
- **documentation** — the only server with any outbound network capability, and it's
  ...
  `list_tools()` (`claudenv/adapters/mcp/documentation/server.py,81-89`). A tier-2/3 agent
...
- **`call_tool()` never leaks a raw exception to the agent.** filesystem-policy wraps
  ...
  (`claudenv/adapters/mcp/filesystem/server.py`); memory-graph does the same with a
...
- **`terminal`'s environment is scrubbed for every execution path**, both the
  ...
  "LC_ALL", "TMPDIR", "VIRTUAL_ENV", "PWD"}` (`claudenv/adapters/mcp/terminal/server.py`) — no
...
- **`documentation.fetch`'s tier comes from an env var the server trusts as-is**
  ...
  `claudenv/_data/config/mcp-servers.json`'s env block) is responsible for setting it correctly.
  Contrast with `terminal`'s `_repo_tier()` (`claudenv/adapters/mcp/terminal/server.py`),
...
- **`memory-graph` degrades gracefully without an embedder.** If `rag.config.get_embedder()`
  ...
  (`claudenv/adapters/mcp/memory_graph/server.py`).
- **`git.commit` does not exist as a tool in this server**, despite
  `claudenv/_data/config/mcp-servers.json` listing `git.commit` under `requires_approval`. The
  `scopes` array (`claudenv/_data/config/mcp-servers.json`) only lists `log`, `diff`, `blame`,
...
- **`lancedb-rag` and `memory-graph` both cache expensive objects at module scope.**
  ...
  loads once per pair (`claudenv/adapters/mcp/lancedb_rag/server.py`); `memory-graph`
