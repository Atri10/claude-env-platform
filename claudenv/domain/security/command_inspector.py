"""
claude-env :: Domain - Security - Command-string inspection for policy enforcement

Shared command-string parsing + deny-check logic extracted from the
pre-refactor ``security/command_inspector.py`` (flat layout) so the SAME
parsing protects both Claude Code's native Bash tool (via
``claudenv/adapters/hooks/policy_hook.py``, which still owns the
native-tool-specific control-plane guard, destructive-command hard deny, and
inline-secret-in-command-string check) and, potentially, any other command
runner that needs to mechanically enforce policy on an unattended/unapproved
command string. There is currently no such second caller: an
unattended-execution tool (``mcp-servers/terminal/server.py``'s planned
``terminal.run_scratch``) was implemented against this module and then
...
Purity note: this is pure domain logic (no persistence, no network, no audit
side effects). The single ``(base / tok).exists()`` call in
``bash_candidates`` is a *boundary stat* -- a defensive filesystem probe that
gates conservative path detection so arbitrary grep patterns / subcommand
names are not mistaken for files. It is not persistence I/O and touches no
audit ledger; callers that need full purity for an unattended path can inject
an ``IPathProbe`` port, but that is over-engineering for the current single
(advisor) consumer, so the stat is kept here with this comment.
"""
    ...
    for seg in segments:
        ...
        for tok in seg[1:]:
            ...
            if looks_like_path(tok):
                ...
                # Boundary stat (see module docstring): gates conservative path
                # detection so a bare slashless token is only admitted as a
                # candidate path when it is a file-command argument that
                # actually exists. Not persistence I/O; no audit side effect.
                paths.append(tok)
    return paths, nets
