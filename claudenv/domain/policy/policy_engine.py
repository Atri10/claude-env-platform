...
class PolicyEngine:
    ...
    Incident-mode is injected as a boolean at construction (read by an
    adapter via IIncidentStore) so the domain layer never touches the
    filesystem.
    """

    def __init__(self, compiled: CompiledPolicy, incident_active: bool = False):
        self.compiled = compiled
        self._incident_active = incident_active
    ...
    def from_yaml(
        ...
        return cls(compiled, incident_active=incident_active)

    def evaluate_path(self, path: Path | str) -> PolicyDecision:
        """Evaluate a path against the policy.

        During incident mode every evaluation fails closed: operations must
        be denied, not merely policy-scored. This is the single choke point
        that makes the filesystem MCP server, the RAG indexer, and any
        other consumer that routes through the engine deny everything while
