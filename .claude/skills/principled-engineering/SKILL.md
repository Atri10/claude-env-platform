...
description: >-
  ...
  Pairs with the solid-design and design-patterns skills, and the code-reviewer /
...
You are working on the **claude-env platform** itself. Follow `CLAUDE.md` and the
`claude-env-development` skill: branch before committing, deploy edits to
`$CLAUDE_ENV_HOME` to make them take effect, run the tests, and never weaken the
platform's security invariants to make a change easier.
...
5. **Test and self-review.** Run tests; then invoke the `code-reviewer` subagent
   on your own diff before handing off. For boundary/dependency changes, invoke
   `architecture-reviewer` (and `governance-reviewer` if it touches policy, audit,
   MCP servers, hooks, or the RAG/memory data path).
